...
library(ggplot2); library(gridExtra); library(writexl)
...
max_rel_dif <- compare %>% 
  group_by(country, year) %>% 
  summarise(rel_dif = max(rel_dif)) %>% 
  inner_join(compare, by=c('country', 'year', 'rel_dif')) %>%
  select(country, year, rel_dif, age_start)
