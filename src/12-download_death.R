...
library(httr); library(readr); library(yaml); library(glue)
...
writeBin(
  ...
  con = glue('{paths$output$tmp}/stmf.zip')
)

# list all files in archive
dat$stmf_filenames <- unzip(
  glue('{paths$output$tmp}/stmf.zip'),
  list = TRUE
)[['Name']]
...
dat$stmf <-
  dat$stmf_filenames %>%
  map(~{
    unz(glue('{paths$output$tmp}/stmf.zip'), filename = .x) %>%
      read_csv(
...
writeBin(
  ...
  con = glue('{paths$output$tmp}/ons_annual_deaths.xlsx')
)
...
dat$ons_annual_deaths <-
  bind_rows(
    male = read_xlsx(
      path = glue('{paths$output$tmp}/ons_annual_deaths.xlsx'),
...
saveRDS(dat$ons_annual_deaths, file = paths$output$ons)
