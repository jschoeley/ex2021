...
paths$output <- list(
  ...
  wpp = './dat/wpp/wpp_population.rds',
  hmdhfd_gb = './dat/hmdhfd/hmdhfd_gb_projection.rds'
)
...
dat$wpp_get <- map(
  c(wpp_estimates = paths$input$url_wpp_estimates,
    wpp_projections = paths$input$url_wpp_projections), ~{
  GET(url = .x, progress())
})
...
saveRDS(
  dat$wpp_df,
  file = paths$output$wpp
)

saveRDS(
  dat$gb_projection_export,
  file = paths$output$hmdhfd_gb
)
