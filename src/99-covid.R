...
data <- read_rds("./out/arriaga_cntfc.rds") %>% 
  mutate(age = as.numeric(age)) %>% 
  select(e0_cntrb_t_mean, year, age, sex, region_iso) %>% 
  filter(year == "2021",
         age >= 60,
         sex == "T")
  group_by(region_iso) %>% 
...
final <- merge(data, vacc, all=TRUE)
...
final %>% 
  ggplot() +
  geom_point(mapping = aes(x = rate_v2_int , y = rate_v2_int, size = vac_int))+
...
final %>% 
  ggplot() +
  geom_point(mapping = aes(x = rate_v2_aug , y = e0_cntrb_t_mean))+
  geom_text_repel(mapping = aes(x = rate_v2_aug , y =  e0_cntrb_t_mean, label = region_iso)) +
  # scale_shape_manual(values=1:36) +
  scale_size_area()+
  geom_abline()+
  labs(x="Percentage Vaccinated 60+ at the end of August", y="Change in e0 from 2020 to 2021 in months", 
...
data2 %>% 
  ggplot() +
  geom_point(mapping = aes(x = rate_v2_int, y = rate_v2_int, color = Country, shape = Country)) +
